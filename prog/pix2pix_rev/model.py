...
#  Upsampling - смотреть https://arxiv.org/abs/1609.05158
...
def crt_D():
    ...
    # поканальная конкатенация двух тензоров
...
def crt_G():
    ...
    # U-Net
    x9 = decode_layer(x = x8, y = x7, k = 4, mult = 8)#, drop=True)
    x10 = decode_layer(x = x9, y = x6, k = 4, mult = 8)#, drop=True)
    x11 = decode_layer(x = x10, y = x5, k = 4, mult = 8)#, drop=True)
...
#m = crt_G()
#print(m.summary())
#tf.keras.utils.plot_model(m, show_shapes=True, dpi=64)
